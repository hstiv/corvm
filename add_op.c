...
/*   Updated: 2019/07/26 16:34:20 by sdiedra          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	op_live(t_vm *vm, t_proc *proc)
{
	...
	number = reverse_bytes(vm, (proc->pos + 1) % MEM_SIZE, 4);
	proc->live = vm->cycles;
...
void	op_and(t_vm *vm, t_proc *proc)
{
	...
	while (++i < 2)
	{
		...
		if (type == T_REG)
			...
			args[i] = reverse_bytes(vm, (proc->pos + 2 + j) % MEM_SIZE, 4);
		else
		{
...
void	op_ldi(t_vm *vm, t_proc *proc)
{
	...
	while (++i < 2)
	{
		...
		if (type == T_REG)
			...
			args[i] = reverse_bytes(vm, (proc->pos + 2 + j) % MEM_SIZE, 2);
		else
		{
			args[i] = reverse_bytes(vm, (proc->pos + 2 + j) % MEM_SIZE, 2) % IDX_MOD;
			args[i] = plus_pos(args[i], proc->pos);
...
void	op_lld(t_vm *vm, t_proc *proc)
{
	...
	{
		n_pos = reverse_bytes(vm, (proc->pos + 2) % MEM_SIZE, 2);
